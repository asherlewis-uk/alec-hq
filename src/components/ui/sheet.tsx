...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ref={ref}
      {...designMarker("SheetContent")}
      className={cn(sheetVariants({ side }), transitionClass(), className)}
      {...props}
    >
      {children}
      <SheetPrimitive.Close
        className={cn(
          "absolute right-4 top-4 rounded-full border border-white/10 bg-white/5 p-1.5 opacity-80 hover:opacity-100 disabled:pointer-events-none",
          transitionClass(),
          focusRingClass()
        )}
      >
        <X className={cn("h-4 w-4", textToneClass("secondary"))} />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn("text-lg font-semibold", textToneClass("primary"), className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description
    ref={ref}
    className={cn("text-sm", textToneClass("secondary"), className)}
    {...props}
  />
))
