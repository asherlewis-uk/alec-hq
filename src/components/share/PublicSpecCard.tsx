      ...
      <div className="max-w-2xl mx-auto">
        ...
        >
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <h1 className="text-4xl font-bold text-primary">{asset.name}</h1>
            </div>
          </div>
        ...
        {components.length > 0 ? (
            ...
            <div className="space-y-8">
              {components.map((component) => (
                <div key={component.id} className="border-l-2 border-accent/30 pl-6">
                  <h3 className="text-lg font-semibold text-primary">{component.name}</h3>
                  {component.brand && (
                    <p className="text-sm text-secondary">{component.brand}</p>
                  )}
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-4">
                      {Object.entries(component.specs).map(([key, value]) => (
                        <div key={key}>
                          ...
                          <p className="text-base text-primary font-mono mt-1">{value}</p>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {component.notes && <p className="text-xs text-muted mt-3 italic">{component.notes}</p>}
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <div className="glass rounded-glass p-8 text-center">
            <p className="text-secondary">No specifications added yet</p>
          </div>
        )}
        ...
        >
          <p className="text-xs text-muted">Built with ALEC.HQ</p>
        </motion.div>
      </div>
    </motion.div>
  )
