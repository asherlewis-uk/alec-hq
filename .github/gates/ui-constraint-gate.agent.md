...
Ensure all UI adheres strictly to the ALEC.HQ design system defined in `tailwind.config.ts`.
...
All UI must use tokens defined in `tailwind.config.ts`.
...
* No inline styles
* No hardcoded color values outside the approved gradient
